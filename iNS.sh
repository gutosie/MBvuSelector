...
#
    ...
    if [ -f /usr/bin/curl ] ; then    
        ...
        curl -O --ftp-ssl https://raw.githubusercontent.com/gutosie/MBvuSelector/master/imbvuver > /dev/null 2>&1
    fi
    if [ ! -e /tmp/MBvuSelector.zip ] ; then
        if [ -f /usr/bin/wget ]; then 
           wget --no-check-certificate $URL
           wget -q --no-check-certificate https://raw.githubusercontent.com/gutosie/MBvuSelector/master/imbvuver > /dev/null 2>&1
           $MVmain
        fi
    fi
    if [ ! -e /tmp/neoscript.zip ] ; then    
        if [ -f /usr/bin/fullwget ]; then 
           ...
           fullwget -q --no-check-certificate https://raw.githubusercontent.com/gutosie/MBvuSelector/master/imbvuver > /dev/null 2>&1
           $MVmain
        fi           
    fi    
